...
//                list($width, $height) = getimagesize($_FILES["myfile"]["tmp_name"]);
                ...
                for ($i = 0; $i < $fileCount; $i++) {
                    ...
                    if (move_uploaded_file($_FILES["myfile"]["tmp_name"][$i], $output_dir . $new_name)) {
                        ...
                        Imagick::open($output_dir . '/' . $new_name)->crop($x, $y, $w, $h)->saveTo($output_dir . $new_name);
                        $img = Imagick::open($output_dir . $new_name)->thumb(110, 110)->saveTo($output_dir . 'thumbnails/' . $new_name);
